<?php
    ...
    // аргумент, соответствующий суперглобальной переменной $_GET
        private array $get,
    // аргумент, соответствующий суперглобальной переменной $_SERVER
...
// HTTP-метод хранится под ключом REQUEST_METHOD
    ...
    // Декодируем в ассоциативный массив
                    associative: true,
    ...
    // Напрмер, для http://example.com/some/page?x=1&y=acb // путём будет строка '/some/page'
        /**
         * @throws HttpException
         */
    ...
    // значение URI хранится под ключом REQUEST_URI
            ...
            if (!is_array($components) || !array_key_exists('path', $components)) { // Если мы не можем получить путь - бросаем исключение
                throw new HttpException('Cannot get path from the request');
            }
            return $components['path'];
        }
        // Метод для получения значения
        // определённого параметра строки запроса
    ...
    // а знаки подчёркивания заменены на минусы
